<?php 
	if (isset($_SESSION['IDNO'])) {
		...
		$sql = "SELECT DISTINCT * FROM `tblstudent` s, `course` c WHERE s.`COURSE_ID`=c.`COURSE_ID` AND IDNO=".$_SESSION['IDNO'];
...
$mydb->setQuery("SELECT DISTINCT * FROM `subject` s, `course` c WHERE s.COURSE_ID=c.COURSE_ID AND s.COURSE_ID=".$_SESSION['COURSEID']." AND SEMESTER='".$_SESSION['SEMESTER']."'");
;
			$res = @mysqli_query($mydb->conn,$sql);
           while($row = @mysqli_fetch_assoc($res)) {
             # code...
			 $mydb->setQuery("
    SELECT DISTINCT s.SUBJ_ID, s.SUBJ_CODE, s.SUBJ_DESCRIPTION, s.UNIT, s.PRE_REQUISITE, c.COURSE_NAME, c.COURSE_LEVEL, s.SEMESTER
    FROM `subject` s
    JOIN `course` c ON s.COURSE_ID = c.COURSE_ID
    WHERE s.COURSE_ID = ".$_SESSION['COURSEID']." 
    AND s.SEMESTER = '".$_SESSION['SEMESTER']."'
");
...
if ($cur) {
    foreach ($cur as $result) {
        ...
        echo '<td align="center"><a title="Add" href="validatingsubject.php?id=' . $result->SUBJ_ID . '" class="btn btn-primary btn-xs">Add <span class="fa fa-plus fw-fa"></span></a></td>';
        echo '</tr>';
    }
} else {
    echo '<tr><td colspan="8">No subjects found for the selected course and semester.</td></tr>';
}
